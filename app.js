...
function checkPalindromeDate(date) {
  ...
  if (Number(counter1) < Number(counter2)) {
    ...
    displayMessage(
      `Your next nearest date is ${nextDate.day}-${nextDate.month}-${nextDate.year}. You have to wait by next ${counter1} ${days}.`,
    );
  } else {
    let days = counter2 === 1 ? ' Day' : ' Days';
    displayMessage(
      `Your previous nearest date is ${prevDate.day}-${prevDate.month}-${prevDate.year}. You missed by ${counter2} ${days}.`,
    );
  }
}
